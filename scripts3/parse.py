...
def get_difficulty(file):
    ...
    if (
        "This course was conceptually challenging compared to most courses I have taken at SCU"
        in _survey
    ):
        ...
        pattern3 = r"Much more[\s[0-9]*n=[0-9]*\s*av.=(.*?)\s*md=(.*?)\s*dev.=(.*?)\s"

        if len(re.findall(pattern, _survey)) > 0:
            difficulty_avg, difficulty_med, difficulty_std_dev = re.findall(
                pattern, _survey
            )[0]
        elif len(re.findall(pattern2, _survey)) > 0:
            difficulty_avg, difficulty_med, difficulty_std_dev = re.findall(
        ...
        elif len(re.findall(pattern3, _survey)) > 0:
            difficulty_avg, difficulty_med, difficulty_std_dev = re.findall(
                pattern3, _survey
            )[0]
        else:
            raise Exception(f"Could not get difficulties from file: {file}")
...
def get_time_spent(file):
    ...
    pattern = (
        r"2.1\) In an average week, the approximate number of hours I spent doing work for this course outside regularly scheduled class/lab time was:"
